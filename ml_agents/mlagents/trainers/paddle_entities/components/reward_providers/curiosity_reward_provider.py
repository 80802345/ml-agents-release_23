...
from mlagents.trainers.paddle_entities.layers import LinearEncoder, linear_layer
...
class ActionPredictionTuple(NamedTuple):
    continuous: paddle.Tensor
    discrete: paddle.Tensor
...
class CuriosityRewardProvider(BaseRewardProvider):
    ...
    def __init__(self, specs: BehaviorSpec, settings: CuriositySettings) -> None:
        ...
        # Paddle handles device placement globally, usually no need for manual .to()
    ...
    def evaluate(self, mini_batch: AgentBuffer) -> np.ndarray:
        ...
        # rewards shape check might be needed, usually [batch_size]
...
class CuriosityNetwork(nn.Layer):
    EPSILON = 1e-10
    ...
    def __init__(self, specs: BehaviorSpec, settings: CuriositySettings) -> None:
        ...
        self.inverse_model_action_encoding = nn.Sequential(
        ...
        self.forward_model_next_state_prediction = nn.Sequential(
            LinearEncoder(
    ...
    def get_current_state(self, mini_batch: AgentBuffer) -> paddle.Tensor:
        """
    ...
    def get_next_state(self, mini_batch: AgentBuffer) -> paddle.Tensor:
        """
    ...
    def predict_action(self, mini_batch: AgentBuffer) -> ActionPredictionTuple:
        ...
        """
        inverse_model_input = paddle.concat(
            (self.get_current_state(mini_batch), self.get_next_state(mini_batch)), axis=1
        )
        ...
        if self._action_spec.discrete_size > 0:
            ...
            branches = [paddle.nn.functional.softmax(b, axis=1) for b in branches]
            discrete_pred = paddle.concat(branches, axis=1)
        return ActionPredictionTuple(continuous_pred, discrete_pred)

    def predict_next_state(self, mini_batch: AgentBuffer) -> paddle.Tensor:
        """
        ...
        forward_model_input = paddle.concat(
            (self.get_current_state(mini_batch), flattened_action), axis=1
        )

        return self.forward_model_next_state_prediction(forward_model_input)

    def compute_inverse_loss(self, mini_batch: AgentBuffer) -> paddle.Tensor:
        """
            ...
            _inverse_loss += paddle.mean(
                ModelUtils.dynamic_partition(
                    ...
                    ModelUtils.list_to_tensor(
                        mini_batch[BufferKey.MASKS], dtype='float32'
                    ),
                    2,
                )[1]
            )
        if self._action_spec.discrete_size > 0:
            true_action = paddle.concat(
                ModelUtils.actions_to_onehot(
                    actions.discrete_tensor, self._action_spec.discrete_branches
                ),
                axis=1,
            )
            cross_entropy = paddle.sum(
                -paddle.log(predicted_action.discrete + self.EPSILON) * true_action,
                axis=1,
            )
            _inverse_loss += paddle.mean(
                ModelUtils.dynamic_partition(
    ...
    def compute_reward(self, mini_batch: AgentBuffer) -> paddle.Tensor:
        """
    ...
    def compute_forward_loss(self, mini_batch: AgentBuffer) -> paddle.Tensor:
        """
