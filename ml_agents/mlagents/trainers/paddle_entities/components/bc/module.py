...
class BCModule:
    ...
    def _behavioral_cloning_loss(
        ...
        expert_actions: paddle.Tensor,
    ) -> paddle.Tensor:
        bc_loss = 0
        if self.policy.behavior_spec.action_spec.continuous_size > 0:
            bc_loss += paddle.nn.functional.mse_loss(
        ...
        if self.policy.behavior_spec.action_spec.discrete_size > 0:
            ...
            bc_loss += paddle.mean(
                paddle.stack(
                    [
                        paddle.sum(
                            -paddle.nn.functional.log_softmax(log_prob_branch, axis=1)
    ...
    def _update_batch(
        ...
        if self.policy.use_recurrent:
            # paddle.zeros
            memories = paddle.zeros([1, self.n_sequences, self.policy.m_size], dtype='float32')
