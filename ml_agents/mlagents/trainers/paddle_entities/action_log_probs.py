...
class ActionLogProbs(NamedTuple):
    ...
    :param continuous_tensor: Paddle tensor corresponding to log probs of continuous actions
    :param discrete_list: List of Paddle tensors each corresponding to log probs of the discrete actions that were
    sampled.
    :param all_discrete_list: List of Paddle tensors each corresponding to all log probs of
    ...
    all_discrete_list: Optional[List[paddle.Tensor]]
    ...
    def to_log_probs_tuple(self) -> LogProbsTuple:
        ...
        if self.discrete_list is not None:
            # 这里的 discrete_tensor 调用了上面的 property，返回的是 stacked tensor
            discrete = ModelUtils.to_numpy(self.discrete_tensor)
            log_probs_tuple.add_discrete(discrete)
        return log_probs_tuple

    def _to_tensor_list(self) -> List[paddle.Tensor]:
        """
        Returns the tensors in the ActionLogProbs as a flat List of Paddle Tensors. This
        is private and serves as a utility for self.flatten()
        """
        tensor_list: List[paddle.Tensor] = []
        if self.continuous_tensor is not None:
    ...
    def flatten(self) -> paddle.Tensor:
        """
